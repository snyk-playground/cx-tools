...
def group_orgs(headers, api_ver, grp, pagination):
    if pagination is None:
        ...
        url = 'https://api.snyk.io/rest/groups/{0}/orgs?version={1}&starting_after={2}'.format(grp['id'], api_ver, pagination)
...
def org_projects(headers, api_ver, org, project_tags, pagination):
    ...
    if pagination is None:
        ...
        url = 'https://api.snyk.io/rest/orgs/{0}/projects?version={1}&tags={2}&starting_after={3}'.format(org['id'], api_ver,
                                                                                       urllib.parse.quote(project_tags.replace(" ","")), pagination)
    response = requests.request("GET", url, headers=headers)
    return response.text
